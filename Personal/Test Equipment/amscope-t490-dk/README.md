# AmScope T490-DK

**Simul-Focal Darkfield Trinocular Compound Microscope**

## Unit Information

| Field | Value |
|-------|-------|
| Manufacturer | AmScope |
| Model | T490B-DK |
| Type | Compound Microscope |
| Head Type | Siedentopf Trinocular |
| Status | Working |
| Date Ordered | 2016-02-11 |
| Purchase Price | $341.98 |
| Source | Amazon |

## Specifications

| Specification | Value |
|--------------|-------|
| Magnification Range | 40X - 2000X |
| Eyepieces | WF10X/18 and WF20X |
| Stock Objectives | 4X, 10X, 40X (S), 100X (S, Oil) Achromatic |
| Head Angle | 30° inclined |
| Interpupillary Distance | 55-75mm |
| Diopter Adjustment | Both tubes |
| Condenser | Dry Darkfield Condenser NA 0.90 |
| Illumination | 20W Halogen |
| Stage Type | 2-layer mechanical, 3D |
| Stage Size | 140mm x 140mm |
| Stage Movement | 75mm x 50mm |
| Focus | Coaxial coarse and fine |
| Fine Focus Precision | 0.002mm |

## Additional Objectives

| Set | Objectives | Source |
|-----|------------|--------|
| Plan Olympus | 10X, 20X, 40X, 100X HI (oil) | eBay (dcimaging) $56.70 - May 2025 |
| Achromatic | 1X, 2X (RMS, 45mm) | eBay (augua_66) $37.44 - May 2025 |

## Features

- Simul-focal trinocular head (camera port usable with both eyepieces)
- Darkfield illumination for enhanced contrast
- Compensation-free Siedentopf head design
- Precision focus with tension adjustment and vertical lock
- Low-position stage controls for ergonomics
- C-mount compatible trinocular port

## Applications

- Blood work and fluid analysis
- Pathology
- Veterinary diagnostics
- Cell biology
- Microbiology

## Accessories

### Included
- WF10X/18 eyepieces (pair)
- WF20X eyepieces (pair)
- Dry darkfield condenser
- Dust cover
- Extra halogen bulb
- Immersion oil

### Purchased Separately
| Item | Price | Source |
|------|-------|--------|
| AC-B400 Aluminum Case | $112.70 | Amazon (Feb 2016) |
| CA-NIK-SLR Nikon Adapter | $89.75 | Amazon (Feb 2016) |
| OMAX Stage Micrometer 0.01mm | $9.99 | Amazon (Feb 2016) |
| Slides (72) + Coverslips (100) | $12.98 | Amazon (Feb 2016) |

## Documentation

- [AmScope T490-DK Product Page](https://amscope.com/products/c-t490-dk)
- [AmScope Support](https://amscope.com/pages/support)

## Camera Options

### AmScope MU500-HS (shared with SM-3)

| Specification | Value |
|--------------|-------|
| Sensor | 5MP CMOS |
| Resolution | 2592 x 1944 |
| Interface | USB 2.0 (high-speed) |
| Mount | C-mount |
| Reduction Lens | FMA050 (0.5X) |

Features:
- High-speed preview with independent capture resolution
- Extended Depth of Focus (EDF)
- Image stitching
- Measurement tools
- Video recording

### Nikon F-Mount Adapter (shared with SM-3)

| Specification | Value |
|--------------|-------|
| Model | NDPL-1 |
| Magnification | 2X |
| Mount | Nikon F-mount |

Allows use of Nikon DSLR/mirrorless cameras on the trinocular port.

## Software

- **AmScope Software** - Image capture and analysis for Windows
  - [Download](https://amscope.com/pages/software-downloads)
- **AmScope Lite** - Basic version for Mac/Linux

## Notes

Trinocular head supports C-mount cameras. The simul-focal design allows simultaneous viewing through eyepieces while using the camera port.

## Local Files

- `T490_UserManual.pdf` - 490 Series Operator's Manual (497KB)

## Revision History

| Date | Change |
|------|--------|
| 2025-12-24 | Initial entry |
| 2025-12-30 | Added 1X/2X objective purchase info |
| 2025-12-30 | Added purchase info and accessories |
